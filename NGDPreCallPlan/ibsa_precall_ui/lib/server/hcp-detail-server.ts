...
export async function getHCPDetailFromRow(row: any, npiParam: string): Promise<HCPDetail> {
  ...
  // Map NGD category from numeric predictions (0=Decliner, 1=Stable, 2=Grower, 3=New)
  const mapNGDCategoryFromNumber = (pred: any): 'New' | 'Grower' | 'Stable' | 'Decliner' => {
    const num = Number(pred)
    if (num === 0) return 'Decliner'
    if (num === 1) return 'Stable'
    if (num === 2) return 'Grower'
    if (num === 3) return 'New'
    return 'Stable'  // fallback
  }

  // Map NGD category from text (fallback)
  const mapNGDCategory = (pred: any): 'New' | 'Grower' | 'Stable' | 'Decliner' => {
    if (typeof pred === 'number') {
      return mapNGDCategoryFromNumber(pred)
    }
  ...
  // Get product-specific predictions (FIXED: Use new column names)
  ...
  return {
    ...
    call_success_score: Number(row.growth_probability) || 0,
    value_score: Number(row.expected_roi) || 0,
    ngd_decile: 5,
    ngd_classification: mapNGDCategory(row.Tirosint_ngd_category_pred) || 'Stable',
    tirosint_wallet_growth: Number(row.tirosint_wallet_growth) || 0,
    flector_wallet_growth: Number(row.flector_wallet_growth) || 0,
    licart_wallet_growth: Number(row.licart_wallet_growth) || 0,
    total_wallet_growth: Number(row.total_wallet_growth) || 0,
    address: '',
    phone: '',
    ...
    predictions: {
      ...
      growth_probability: bestCallSuccess,  // FIXED: Renamed from growth_probability
