...
namespace SFBulkAPIStarter
{
    ...
    public class BulkApiClient
    {
        ...
        public Job CreateJob(CreateJobRequest createJobRequest)
        {
            ...
            String createJobUrl = "https://" + _sfService.Pod + ".salesforce.com/services/async/" + _sfService.Version + "/job";

            String resultXML = invokeRestAPI(createJobUrl, jobRequestXML);

            return new Job().Create(resultXML);
        }
        ...
        public List<BulkAPIResult> GetBulkAPIResult(string resultJSON)
        {
            var resultList = new List<BulkAPIResult>();
            if (!string.IsNullOrEmpty(resultJSON))
            {
                var rows = resultJSON.Split(Environment.NewLine.ToCharArray()).Skip(1);
                foreach (var row in rows)
                {
                    ...
                    else if(columns.Length > 4)
                    {
                        resultList.Add(new BulkAPIResult
                        {
                            ...
                            Success = bool.Parse(columns[1].Trim('"')),
                            Created = bool.Parse(columns[2].Trim('"')),
                            Error = row.Substring(row.IndexOf(columns[3].Trim('"')))
                        });
                    }
                }
            }
        ...
        public Batch CreateAttachmentBatch(CreateAttachmentBatchRequest request)
        {
            ...
            using (MemoryStream memoryStream = new MemoryStream())
            {
                using (var archive = new ZipArchive(memoryStream, ZipArchiveMode.Create, true))
                {
                    ...
                    String requestUrl = "https://" + _sfService.Pod + ".salesforce.com/services/async/" + _sfService.Version + "/job/" + request.JobId + "/batch";
        ...
        {
            String requestUrl = "https://" + _sfService.Pod + ".salesforce.com/services/async/" + _sfService.Version + "/job/" + createBatchRequest.JobId + "/batch";
        ...
        {
            String requestUrl = "https://" + _sfService.Pod + ".salesforce.com/services/async/" + _sfService.Version + "/job/" + jobId + "/batch/" + batchId;
        ...
        public string GetBatchResult(string jobId, string batchId)
        {
            String requestUrl = "https://" + _sfService.Pod + ".salesforce.com/services/async/" + _sfService.Version + "/job/" + jobId + "/batch/" + batchId + "/result";
        ...
        {
            String requestUrl = "https://" + _sfService.Pod + ".salesforce.com/services/async/" + _sfService.Version + "/job/" + jobId + "/batch/";
        ...
        {
            String requestUrl = "https://" + _sfService.Pod + ".salesforce.com/services/async/" + _sfService.Version + "/job/" + jobId + "/batch/" + batchId + "/request";
        ...
        {
            String requestUrl = "https://" + _sfService.Pod + ".salesforce.com/services/async/" + _sfService.Version + "/job/" + jobId + "/batch/" + batchId + "/result/" + resultId;
        ...
        {
            return "https://" + _sfService.Pod + ".salesforce.com/services/async/" + _sfService.Version + "/job/" + jobId;
        }
